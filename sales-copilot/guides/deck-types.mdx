...
Tier display order (left to right on the rendered slide) is controlled by the editor's drag-to-reorder handles and persisted with the slide. The `product-data` payload exposes this as a `tierOrder` array (for example, `["better", "good", "best"]`); when omitted or invalid, the slide falls back to the default `good`, `better`, `best` order.
