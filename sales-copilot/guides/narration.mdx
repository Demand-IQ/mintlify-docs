---
title: Narration & Voices
description: Choose the voice for your presentations and keep narration up to date as you edit slides.
---

Every slide in a Sales CoPilot presentation is narrated by a synthesized voice. The audio plays automatically as slides advance, with the script text highlighted word-by-word in sync.

## Choosing a voice

You set the voice when creating a deck. In the **New Deck** dialog, you'll see a **Use Premium Voice AI** toggle. This controls the voice quality used across all slides, FAQs, and Q&A responses in the deck.

Pick the option that fits your use case — premium voices sound more natural but take slightly longer to generate.

## Regenerating audio after edits

If you update a slide's narration script, the audio needs to be regenerated to match. Open the slide editor and click **Regenerate Audio from Script** after making your changes. The updated audio replaces the old version.

If you'd rather have the AI rewrite the script entirely from the slide image, click **Use AI to Generate Script** — this rewrites the narration based on what's in the image, using your deck context as guidance.

## FAQ and Q&A audio

Audio for FAQ answers is generated automatically when you save a FAQ. If you edit the answer later, it regenerates automatically.

For the Q&A transition and resume prompts (in the **FAQs** tab under **Q&A Experience Settings**), audio is not automatic — after saving the prompt text, click **Generate Audio** to produce the spoken version.

## Checking audio status

Throughout the deck editor you'll see status indicators on audio content:

- **Audio Ready** — the spoken audio is generated and ready
- **Generating...** — audio is currently being produced
- **Failed** — generation didn't complete; try regenerating

Each slide and FAQ has an audio preview player so you can listen before sending anything to a homeowner.

## Preview audio for personalized slides

If your narration scripts use personalization tokens like `{{first_name}}` or `{{address}}`, Sales CoPilot generates separate preview audio with placeholder values so you can hear how the slide will sound without creating a presentation. The deck editor shows a banner when slides need preview audio generated — click **Generate Preview Audio** to create it for all applicable slides at once.
