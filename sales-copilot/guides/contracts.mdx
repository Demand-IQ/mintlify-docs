...
A deck can have multiple action configs (contract templates), but only one should be published at a time. Pass the `configId` of the template you want to sign against when calling the signing endpoint. If you omit `configId`, Sales CoPilot signs against a built-in default template — set `configId` explicitly to use your deck's published contract.
