...
Upload a PDF or Word document (max 10 MB). Sales CoPilot uses AI to analyze the document and convert it to a markdown template. It detects variables like `{{full_name}}` and `{{address}}` that get replaced with the homeowner's real information when the contract is presented.
...
Both verbs require an `x-presentation-token` header scoped to the presentation, the same token used by `/events` and `/appointment`. Mint one with `POST /api/presentations/{presentationId}/events-token`, then send it on every call. Calls without a valid token return `401 Unauthorized`. Signing a soft-deleted presentation returns `404`, even if the token is still within its 24-hour TTL.
