...
Both verbs are callable without authentication using the `presentationId` in the URL. The typed signature is validated against the homeowner's name on the presentation, so a caller cannot sign under a different name.
