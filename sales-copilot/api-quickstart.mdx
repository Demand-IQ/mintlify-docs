...
Create a personalized presentation for the homeowner. `first_name`, `address`, `city`, and `country` are required; the address is validated via Google Geocoding, so a `422` means it couldn't be resolved:
