...
To add a logo or rep avatar, upload the image first with `POST /api/images/upload` (multipart, `file` field) and pass the returned S3 key as `logo` or `avatar`. Deleting an image with `DELETE /api/images/{imageId}` returns `409 Conflict` while any deck or presentation still references it, along with a list of where it's used. Clear those references first.
