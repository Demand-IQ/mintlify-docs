...
<span id="step-1-authenticate" />
This walkthrough uses a session cookie obtained from `POST /api/auth/login` for template management and presentation creation. Log in once and store the cookie. The examples below use a curl cookie jar (`-b cookies.txt`) to send it on every request:
...
<Step title="Generate slides from your images">
<span id="step-3-generate-slides-from-your-images" />
...
<Step title="Set up branding (recommended)">
<span id="step-4-set-up-branding-recommended" />
...
<Step title="Create and send a presentation">
<span id="step-5-create-and-send-a-presentation" />
Create a personalized presentation for the homeowner. Provide a `homeowner` object (creates a new Core prospect from those details), an existing `prospect_id` (reuses a prospect's contact info; provide pricing inputs separately), or both (attaches the existing prospect but uses the `homeowner` fields you send as-is for personalization). At least one of the two is required. For the `homeowner` path, `first_name`, `address`, `city`, `state`, and `zip` are required; the address is validated via Google Geocoding, so a `422` with code `INVALID_ADDRESS` means it couldn't be resolved. A `422` with code `PROSPECT_NOT_FOUND` means the `prospect_id` isn't visible to your credential, and a `401` with code `CORE_AUTH_REJECTED` means your session or API key doesn't have the required Core prospect scopes.
...
<Note>
`generating` is a valid presentation status. Wait for `ready` here; `completed` belongs to the template-generation job in Step 3.
</Note>
