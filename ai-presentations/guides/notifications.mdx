...
Start with [Get a Presentation URL via API](/ai-presentations/guides/presentation-url-api) to create the presentation you'll track. The [Integration overview](/ai-presentations/integration-overview) connects creation, delivery, and follow-up. Subscription management requires a session cookie or an SSO Bearer session token; a presentation-creation API key does not replace a session here.
...
Signing is optional and must be configured for your subscription; creating a subscription through the API does not provision a signing secret. Contact Demand IQ to arrange signing and obtain the shared secret. Until signing is configured, validate a secret `Authorization` header configured on your subscription over HTTPS. Do not accept an unsigned request as a signed event.

Signed requests include `X-Webhook-Signature: t=<unix-seconds>,v1=<hex-digest>`. Compute HMAC-SHA256 with your shared secret over the timestamp, a period, and the **raw request body**: `timestamp + "." + rawBody`. Compare the hexadecimal digest with `v1` using a constant-time comparison. Reject missing or malformed signatures and timestamps outside your chosen tolerance (for example, five minutes). Parse the JSON only after verification, and deduplicate by `event_id`.

#### Retries and recovery

Return a `2xx` promptly after durably accepting the event. Requests time out after 10 seconds. Responses with `429` or `5xx`, timeouts, and recognized connection failures are retryable; other HTTP errors are terminal.

The current delivery configuration allows five queue processing attempts, normally about two minutes apart (roughly eight minutes from first to fifth attempt). This is not an exact delivery deadline: processing time, backlog, and worker restarts can extend it, and an attempt can fail before reaching your endpoint.

After the retry limit, the event moves to an internal dead-letter queue with 14-day retention. There is no public replay endpoint. Contact Demand IQ with the `event_id` to investigate recovery; changing a subscription alone does not replay a terminal failure. The [event log](#event-log) lets you reconcile activity independently of webhook delivery.

#### Event versions

`event_version` identifies the payload schema for that `event_type`, not the API version or delivery attempt. Version `1` is the initial schema; later versions can add or change fields. Handle the versions your consumer supports explicitly, tolerate unknown optional fields, and review payload changes before relying on new fields. A retry keeps the recorded event version.
