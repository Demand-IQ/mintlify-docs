...
- Each slide is rendered to a WebP image, with a matching WebP thumbnail, and becomes a slide in the new deck in the original order.
...
- Slides are first saved as `Slide 1`, `Slide 2`, and so on. After the `201` response, a background pass reads each rendered image and renames the slide after what is on it. Titles appear on the next slides fetch. If the pass fails or a slide can't be read, that slide keeps its `Slide N` placeholder.
...
PowerPoint files are limited to 25 MB and 50 slides, with a 30 MB cap on the whole request body. Imports are all-or-nothing: if anything fails, no deck is created and the error response tells you why. If a file cannot be converted (`rendering_failed`), re-save it from PowerPoint and try again — that rewrites the package with Microsoft's own writer and clears most conversion failures.

The rendered slide images and thumbnails are stored with the deck and removed when the deck is deleted (`DELETE /api/decks/{deckId}`). The uploaded PowerPoint file itself is not kept on a successful import — it is only stored for a failed import, to help diagnose the failure. See the API Reference for full request and response shapes.
...
The journey block must return at least one product option. Options map positionally onto the `good`, `better`, and `best` tiers in that order: one option fills `good`, two fill `good` and `better`, three fill all three. Any tier the journey doesn't fill is dropped from the slide — no manual card is rendered in its place, and its `{{product.good|better|best.*}}` narration tokens stay unresolved so the surrounding sentence is stripped before text-to-speech rather than narrating a product nobody can see. A journey with zero product blocks is still rejected: a journey-backed slide with no products is an empty slide.

If the journey returns more than three options, the extras render after the tiers in a horizontally scrollable row (with a scrollable pill row on portrait mobile), and homeowners can select any of them — including an extra — as the product carried through to narration, the confirmation modal, and the contract. Extra cards show the tier badge configured on their journey option, the same as the first three cards. Only `{{product.good|better|best.*}}` narration tokens are available for the first three tiers; extras are selectable but not individually addressable by tier token. Use `{{product.selected.*}}` to reference whichever card the homeowner picks. When the stored default selection is a tier the journey didn't fill, `{{product.selected.*}}` falls back to the first card that's actually displayed, matching what the viewer highlights on screen.
